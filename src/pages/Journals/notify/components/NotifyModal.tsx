...
import useNotifyImport from "@/pages/Journals/notify/hooks/useNotifyImport.ts";

interface ImportNotifyModalProps {
  open: boolean;
...
const NotifyModal = ({
  ...
  onOpenChange,
  refetch,
}: ImportNotifyModalProps) => {
  const { t } = useTranslation();
  const inputRef = useRef<HTMLInputElement>(null);
  const [files, setFiles] = useState<File[]>([]);
  const [progress, setProgress] = useState(0);

  const { handleUpload, loading } = useNotifyImport({
    onSuccess: () => {
      setProgress(100);
      setTimeout(() => {
        onOpenChange(false);
