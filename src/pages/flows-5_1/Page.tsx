...
import { CirclePlusIcon, UploadIcon, Trash2Icon, RefreshCw, Layers } from "lucide-react";
...
const Page = () => {
  ...
  return (
      <>
        <PageHeader className={"sticky top-0"} breadcrumbs={breadcrumbs}>
          <div className="flex items-center gap-2">
            <Button
                ...
                onClick={() => setUpdateManyModalOpen(true)}
            >
              <RefreshCw className="size-4" />
              {t("Update all")} ({selectedRowKeys.length})
            </Button>
            <Button
                size={"sm"}
                variant="destructive"
            ...
            <Button size={"sm"} onClick={() => setImportModalOpen(true)} variant="ghost">
        ...
        <UpdateManyFlowsModal
            open={updateManyModalOpen}
            onOpenChange={setUpdateManyModalOpen}
            selectedIds={selectedRowKeys}
            onSuccess={handleUpdateManySuccess}
        />
      </>
  );
};
