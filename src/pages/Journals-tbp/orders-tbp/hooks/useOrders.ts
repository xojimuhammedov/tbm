...
const useOrders = () => {
  ...
  const { removeWithConfirm, remove } = useDelete([ORDERS_QUERY_KEY]);
  const { query, handleFilter, params } = useLists<OrdersInterface>({
    url: [ORDERS_QUERY_KEY],
  });
  ...
  const handleAdd = useCallback(() => {
    navigate("/journals/orders-tbp/create");
  }, [navigate]);

  const handleEdit = useCallback(
    (id: string) => {
      navigate(`/journals/orders-tbp/edit/${id}`);
    },
    [navigate],
  );
