...
import useTimestamp from "@/shared/hooks/e-imzo/useTimestamp";
...
const useEImzoSign = (documentId: string, pdfPath?: string) => {
  ...
  const [pkcs7WithTimestamp, setPkcs7WithTimestamp] = useState<string | null>(
    null,
  );
  ...
  const handleSign = useCallback(async () => {
    ...
    try {
      ...
      setStep("loading-key");
      const loadData = await capiCall<{ id?: string; keyId?: string }>(
        plugin,
        "load_key",
        [
          selectedCert.disk,
      ...
      // 4. PKCS#7 imzolash
      setStep("signing");
      const signData = await capiCall<{ pkcs7_64: string }>(
        "pkcs7",
        "create_pkcs7",
        [base64, loadedKeyId, "no"],
      );

      if (!signData?.pkcs7_64)
        throw new Error(t("Imzolash muvaffaqiyatsiz yakunlandi."));
