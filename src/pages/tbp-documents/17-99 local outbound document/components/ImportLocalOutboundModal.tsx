...
import useLocalOutboundImport from "@/pages/in & out documents/17-99 local outbound document/hooks/useLocalOutboundImport.ts";
...
const ImportLocalOutboundModal = ({
  open,
  onOpenChange,
  refetch,
  ...
  const { handleUpload, loading } = useLocalOutboundImport({
    onSuccess: () => {
      setProgress(100);
      setTimeout(() => {
        onOpenChange(false);
...
export default ImportLocalOutboundModal;
