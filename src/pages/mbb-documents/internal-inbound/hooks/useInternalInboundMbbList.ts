...
import createInternalInboundMbbColumns from "@/pages/mbb-documents/internal-inbound/helpers/createInternalInboundMbbColumns";
import { useFlowDeleteActions } from "@/shared/hooks/flow/useFlowDeleteActions.ts";

const useInternalInboundMbbList = () => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { toast } = useToast();

  const { removeWithConfirm, remove } = useDelete([INTERNAL_INBOUND_MBB_QUERY_KEY]);
  const { query, handleFilter, params } = useLists<InternalInboundMbbDocument>({
    url: [INTERNAL_INBOUND_MBB_QUERY_KEY],
  });
  ...
  const handleDelete = useCallback(
    (id: InternalInboundMbbDocument["_id"]) => {
  ...
  const handleAdd = useCallback(() => {
    navigate("/mbb/mbb-reg-documents/internal-inbound/create");
  }, [navigate]);

  const handleEdit = useCallback(
    (id: string) => {
      navigate(`/mbb/mbb-reg-documents/internal-inbound/edit/${id}`);
    },
    [navigate],
  );
  ...
  const columns = useMemo(
    () =>
      createInternalInboundMbbColumns(
        t,
        handleDelete,
        handleEdit,
        selectedRowKeys,
...
export default useInternalInboundMbbList;
