...
export const createChannelSchema = (
    ...
    z.object({
        code: z.string().nonempty(
            t("required {{field}}", { field: t("Code"), ns: config.LANG.NS.VALIDATION }),
        ),
        flow_id: z.string().nonempty(
            t("required {{field}}", { field: t("Flow ID"), ns: config.LANG.NS.VALIDATION }),
        ),
        flow_code: z.string().nonempty(
            t("required {{field}}", { field: t("Flow Code"), ns: config.LANG.NS.VALIDATION }),
        ),
        international_stream_number: z.string().nonempty(
            t("required {{field}}", { field: t("International stream number"), ns: config.LANG.NS.VALIDATION }),
        ),
        forward_stream_interaction_number: z.string().nonempty(
            t("required {{field}}", { field: t("Forward stream interaction number"), ns: config.LANG.NS.VALIDATION }),
        ),
        reverse_stream_interaction_number: z.string().nonempty(
            t("required {{field}}", { field: t("Reverse stream interaction number"), ns: config.LANG.NS.VALIDATION }),
        ),
        rcpu_site_id_a: z.string().nonempty(
            t("required {{field}}", { field: t("RCPU site id A"), ns: config.LANG.NS.VALIDATION }),
        ),
        icm_a_stream: z.string().nonempty(
            t("required {{field}}", { field: t("ICM A stream"), ns: config.LANG.NS.VALIDATION }),
        ),
        rcpu_site_id_z: z.string().nonempty(
            t("required {{field}}", { field: t("RCPU site id Z"), ns: config.LANG.NS.VALIDATION }),
        ),
        icm_b_stream: z.string().nonempty(
            t("required {{field}}", { field: t("ICM B stream"), ns: config.LANG.NS.VALIDATION }),
        ),
        channel_link: z.string().nonempty(
            t("required {{field}}", { field: t("Channel link"), ns: config.LANG.NS.VALIDATION }),
        ),
        channel_number_in_stream: z.string().nonempty(
            t("required {{field}}", { field: t("Channel number in stream"), ns: config.LANG.NS.VALIDATION }),
        ),
        destination_index: z.string().nonempty(
            t("required {{field}}", { field: t("Destination index"), ns: config.LANG.NS.VALIDATION }),
        ),
        channel_mode: z.string().nonempty(
            t("required {{field}}", { field: t("Channel mode"), ns: config.LANG.NS.VALIDATION }),
        ),
        timeslot_number: z.string().nonempty(
            t("required {{field}}", { field: t("Timeslot number"), ns: config.LANG.NS.VALIDATION }),
        ),
        fac_group_bc: z.string().nonempty(
            t("required {{field}}", { field: t("FAC group BC"), ns: config.LANG.NS.VALIDATION }),
        ),
        site_a: z.string().nonempty(
            t("required {{field}}", { field: t("Site A"), ns: config.LANG.NS.VALIDATION }),
        ),
        ne_a_id: z.string().nonempty(
            t("required {{field}}", { field: t("NE A ID"), ns: config.LANG.NS.VALIDATION }),
        ),
        a_port_nms: z.string().nonempty(
            t("required {{field}}", { field: t("A Port NMS"), ns: config.LANG.NS.VALIDATION }),
        ),
        site_z: z.string().nonempty(
            t("required {{field}}", { field: t("Site Z"), ns: config.LANG.NS.VALIDATION }),
        ),
        ne_z_id: z.string().nonempty(
            t("required {{field}}", { field: t("NE Z ID"), ns: config.LANG.NS.VALIDATION }),
        ),
        z_port_nms: z.string().nonempty(
            t("required {{field}}", { field: t("Z Port NMS"), ns: config.LANG.NS.VALIDATION }),
        ),
