...
const OrderApplicationView1746 = ({
  open,
  onOpenChange,
  document,
  asComponent,
  ...
  const actionType: string = basic?.action_type || "";
  ...
  const getDynamicTitle = () => {
    ...
    return `${directionName} oqimni ${actionText} to'g'risida`;
  };

  const baseIntro = (
    <>
      {basic?.company_name}ning{" "}
      {basic?.request_date
  ...
  // ✅ Block section — alohida component
  const renderBlockSection = (action: "block" | "unblock") => {
    ...
    return (
      <>
              ...
              <div key={i}>
                ...
                <p className="pl-4">
                  ...
                  <span className="font-bold">
                    {String(ch).startsWith("ID") ? ch : `ID${ch}`}
                  </span>
                </p>
              </div>
            ))}
        ...
        {/* Mas'ul */}
        {isBlock && basic?.responsible_3_3 && (
          <p className="mt-3">
            Oqimlarni bloklash bo'yicha mas'ul –{" "}
            <span className="font-bold">{basic.responsible_3_3}</span>
          </p>
        )}
      </>
    );
  ...
  const renderIntroWithSingleAction = () => {
    ...
    // ✅ Block — intro matni flow ID bilan birga
    if (hasBlock) {
      const flowIds: string[] = (payload?.flow_ids || []).map((id: string) =>
        String(id).startsWith("ID") ? String(id) : `ID${id}`,
      );
      const channels: string[] = payload?.channels || [];
  ...
  const renderSections = () => {
    ...
    for (const action of [...actions, actionType].filter(Boolean)) {
      ...
      // ✅ BLOCK (multiple holatda)
      if (action === "block" && hasBlock) {
        step++;
        const flowIds: string[] = (payload?.flow_ids || []).map((id: string) =>
          String(id).startsWith("ID") ? String(id) : `ID${id}`,
        );
        sections.push(
          <div key="block" className="pl-4">
            <p>
              {isMultiple && `${step}. `}
              "O'zbektelekom" AK dagi identifikatori{" "}
              <span className="font-bold">{flowIds.join(", ")}</span> bo'lgan{" "}
      ...
      if (action === "unblock" && hasUnblock) {
        ...
        sections.push(
          <div key="unblock" className="pl-4">
            <p>
              {isMultiple && `${step}. `}
              "O'zbektelekom" AK dagi identifikatori{" "}
              <span className="font-bold">{flowIds.join(", ")}</span> bo'lgan{" "}
  ...
  const renderSingleActionSections = () => {
    ...
    // ✅ Block single action
    if (hasBlock) return renderBlockSection("block");

    // ✅ Unblock single action
    if (hasUnblock) return renderBlockSection("unblock");
  ...
  const DocumentContent = (
    ...
    >
      ...
      <div className="flex justify-between font-bold py-1 text-[14px]">
        <div>
          SANA:{" "}
          {document?.order_date
      ...
      <div className="text-[15px] text-justify space-y-3 mb-10">
        ...
        {/* ✅ 3.3-son shakl mas'ul — basic dan o'qiladi */}
        {basic?.responsible_3_3 && (
          <p className="mt-3">
            3.3-son shakl bo'yicha ma'lumot berishga mas'ul –{" "}
            <span className="font-bold">{basic.responsible_3_3}</span>
          </p>
        )}
      </div>
...
export default OrderApplicationView1746;