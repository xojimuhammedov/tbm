...
import type {OrderApplication, Payload1748} from "@/pages/rh-252/a-252/interfaces/order.interface.ts";
...
const OrderView1748 = ({open, onOpenChange, document}: Props) => {
    ...
    const orderDateText = useMemo(() => {
        return document?.order_date ? dateFormatter(document.order_date, "YYYY-yil DD-MMMM", "uz") : "";
    }, [document?.order_date]);
    ...
    const endOnlyTimeText = useMemo(() => {
        return payload?.basic?.end_time ? dateFormatter(payload.basic.end_time, "HH:mm", "uz") : "";
    }, [payload?.basic?.end_time]);
            ...
            header={
                <div className="flex items-center justify-between w-full pr-12 font-sans">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-gray-800">{t("Farmoyishni ko'rish (17-48)")}</span>
                    </div>
                    <Button
                        onClick={() => handlePrint()}
                        variant="default"
            ...
            <div className="py-10 px-4 flex justify-center bg-gray-100 min-h-screen">
              ...
              `,
                    ...
                    <div className="flex justify-between font-bold py-1 mb-1 text-[14px]">
                        <div>{hasText(orderDateText) ? <>SANA: <span>{orderDateText}</span></> : null}</div>
                        <div>№ {docNumber}</div>
                        <div>SONI: 1</div>
                    </div>
                    ...
                    {(hasText(docTitle) || timeOk || contentOk) ? (
                        <div className="text-[15px] text-justify space-y-2 mb-4">
                                ...
                                <div className="indent-12  m-0">
                                    {timeOk ? (
                                        <>
                                            ...
                                            <span> “2-8” aloqani yopish yoʻli bilan
                            ...
                            {contentOk ? (
                                <p className="w-full">{payload!.content}</p>
                            ) : null}
                        </div>
                    ) : null}
                    ...
                    {stoppedFlowsOk ? (
                        <div className="text-[15px] mb-4">
                            ...
                            <div className="leading-relaxed">
                                {Object.entries(
                                    payload!.stopped_flows.reduce((acc, f) => {
          ...
          <span className="font-bold">ID {codes.join(", ")}</span> ({pointB})
                    ...
                    {includingOk ? (
                        <div className="text-[15px] mb-5">
                            ...
                            <div className="html-content" dangerouslySetInnerHTML={{__html: payload!.including}}/>
                        </div>
                    ) : null}
