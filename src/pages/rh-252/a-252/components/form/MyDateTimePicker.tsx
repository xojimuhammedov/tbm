...
export const MyDateTimePicker: React.FC<Props> = ({ control, name, label }) => {
    ...
    const [datePart, setDatePart] = useState(field.value ? dayjs(field.value).format('YYYY-MM-DD') : '');
    const [timePart, setTimePart] = useState(field.value ? dayjs(field.value).format('HH:mm') : '09:00');
    useEffect(() => {
        if (datePart) {
