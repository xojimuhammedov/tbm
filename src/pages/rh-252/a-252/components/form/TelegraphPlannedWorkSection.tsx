...
const TelegraphPlannedWorkSection = ({ control }: TelegraphPlannedWorkSectionProps) => {
    ...
    return (
        <div className="">
            ...
            <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 mb-2">Qo'shimcha ma'lumot</label>
                <textarea
                    {...control.register("payload.basic.context")}
                    disabled={!isDurationZero}
                    placeholder={isDurationZero ? "Izoh qoldiring..." : "Daqiqa 0 dan katta bo'lgani uchun bu maydon yopiq"}
            ...
            <div className="mt-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {fields.map((field, index) => (
                        <div key={field.id} className="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg hover:border-indigo-300 transition-all group shadow-sm hover:shadow-md">
                            <div className="bg-indigo-100 text-indigo-600 text-[10px] font-bold w-6 h-6 flex items-center justify-center rounded">{index + 1}</div>
                            <div className="flex-1">
                                <MyInput control={control} name={`payload.flow_ids.${index}`} placeholder="ID0000000" className="border-none focus:ring-0 h-8 bg-transparent font-mono text-sm w-full" />
                            </div>
                            <button type="button" onClick={() => remove(index)} className="text-gray-400 hover:text-red-500 p-1 transition-colors">
