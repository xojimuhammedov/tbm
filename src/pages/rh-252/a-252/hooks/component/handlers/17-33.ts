...
const h1733: Handler = {
  populate: (form, payload, ctx) => {
    ...
    form.setValue("payload.basic.deadline", basic.deadline ?? payload.deadline ?? null);
    form.setValue("payload.basic.justification", basic.justification ?? payload.justification ?? "");
    ...
    const delElements = safeArray<any>(deleteSource.elements || deleteSource.flow_ids);
