...
import { ORG_QUERY_KEY } from "../constants/org.constants.ts";
...
import createStaffFilters from "../helpers/createOrgFilters.tsx";
import createOrgColumns from "../helpers/createOrgColumns.tsx";
...
const useOrganization = () => {
  ...
  const { removeWithConfirm } = useDelete([ORG_QUERY_KEY]);
      ...
      removeWithConfirm(id)
        .then(() => {
          ...
          toast({
            variant: "success",
            title: t(`Success`),
            description: t(`Organization removed successfully`),
          });
        })
        .catch((error) => {
          toast({
  ...
  const columns = useMemo(
    () => createOrgColumns(t, handleDelete, handleEdit, handleView),
