...
const Page = () => {
  ...
  useEffect(() => {
    if (docId && sig) {
      const baseUrl =
        import.meta.env.VITE_BASE_URL || "https://eresurs.rtmc.uz";
      axios
        .get<IDocData>(`${baseUrl}/api/rh-252/orderv2/check-qr-code`, {
          ...
          <div className="bg-white shadow-xl rounded-xl sm:rounded-2xl overflow-hidden border border-gray-100">
            ...
            <div className="bg-green-600 p-4 sm:p-6 text-white flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3 text-center sm:text-left">
                ...
                <div>
                  <h1 className="text-lg sm:text-xl font-bold italic tracking-wide uppercase">
                    Hujjat tasdiqlandi
                  </h1>
                ...
                <div className="space-y-3 bg-gray-50/50 p-3 sm:p-5 rounded-lg sm:rounded-xl border border-gray-100 text-xs sm:text-sm">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                    <span className="text-gray-500 flex items-center gap-2">
                      <Clock size={14} /> Imzolangan vaqt:
                    </span>
                    <span className="font-medium text-gray-900 ml-6 sm:ml-0">
                      {formatDate(docData.data.signature_info?.signed_at)}
                    </span>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                    <span className="text-gray-500 flex items-center gap-2">
                      <FileText size={14} /> Sertifikat seriyasi:
                    </span>
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                    <span className="text-gray-500 flex items-center gap-2">
                      <Calendar size={14} /> Amal qilish muddati:
                    </span>
                    <span className="text-gray-700 font-medium ml-6 sm:ml-0">
                      {new Date(
                        docData.data.signature_info?.cert_valid_from,
                      ).toLocaleDateString("uz-UZ")}{" "}
                      —{" "}
                      {new Date(
                        docData.data.signature_info?.cert_valid_to,
                      ).toLocaleDateString("uz-UZ")}
                    </span>
                  </div>
                  <div className="pt-3 border-t border-gray-200 mt-2">
                    <p className="text-[9px] sm:text-[10px] text-gray-400 uppercase font-bold mb-1">
                      Sertifikat beruvchi:
                    </p>
        ...
        {status === "error" && (
          <div className="bg-white shadow-xl rounded-xl p-6 sm:p-10 border-t-4 border-red-500 flex flex-col items-center text-center">
            ...
            <p className="text-sm sm:text-base text-gray-600 max-w-sm mb-6">
              Kechirasiz, ushbu QR kod orqali hujjat topilmadi yoki uning imzosi
              haqiqiy emas.
            </p>
            <button
