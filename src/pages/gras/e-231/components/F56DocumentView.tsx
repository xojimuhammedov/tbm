import { MyModal } from "@/shared/components/moleculas/modal";
import { F56DocumentInterface } from "@/pages/rtsi/f-56/interfaces/f56Document.interface.ts";
import { FileTextIcon } from "lucide-react";
import F56Document from "@/pages/rtsi/f-56/components/F56Document.tsx";
import DocumentInfo from "@/shared/components/moleculas/documents/DocumentInfo.tsx";
import { useTranslation } from "react-i18next";

interface F56DocumentViewProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  document?: F56DocumentInterface | null;
}

const F56DocumentView = ({
  open,
  onOpenChange,
  document,
}: F56DocumentViewProps) => {
  const { t } = useTranslation();
  return (
    <MyModal
      open={open}
      onOpenChange={onOpenChange}
      size="8xl"
      className="overflow-auto"
      header={
        <div className="flex items-center gap-2">
          <FileTextIcon className="h-5 w-5" />
          <span>{document?.title || "F-56 document"}</span>
        </div>
      }
    >
      <div className="flex flex-1 flex-col xl:flex-row gap-4 overflow-hidden">
        <div
          className="
            flex-4/5 border rounded-lg p-2
            overflow-visible
            xl:overflow-y-auto
            xl:max-h-[80vh]
            scrollbar-thin scrollbar-thumb-gray-400 hover:scrollbar-thumb-gray-500 scrollbar-track-gray-100 scrollbar-thumb-rounded-full
          "
        >
          <F56Document title={document?.title} document={document?.documents} />
        </div>

        <DocumentInfo
          docType={t("F56")}
          onOpenChange={onOpenChange}
          document={document}
        />
      </div>
    </MyModal>
  );
};

export default F56DocumentView;
